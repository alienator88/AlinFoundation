...
public class LogStorage: ObservableObject {
    public static var shared: LogStorage? = nil
...
public func printOS(_ items: Any..., separator: String = " ", category: String = LogCategory.general, logType: OSLogType = .error) {
    ...
    if let logStorage = LogStorage.shared {
        let formatter = DateFormatter()
        formatter.dateFormat = "[MMM d, h:mm:ss a]"
        let timestamp = formatter.string(from: Date())
        let datedMessage = "\(timestamp) \(message)"
        logStorage.addLog(datedMessage)
    }
}
...
public struct ConsoleView: View {
    ...
    public init() {
        // Initialize the @StateObject with the shared instance if available, otherwise create a local instance
        if let sharedStorage = LogStorage.shared {
            _logStorage = StateObject(wrappedValue: sharedStorage)
        } else {
            _logStorage = StateObject(wrappedValue: LogStorage())
        }
    }
    ...
    public var body: some View {
        ...
        .onAppear {
            // Initialize LogStorage.shared if not already initialized
            if LogStorage.shared == nil {
                LogStorage.shared = logStorage
            }
        }
        .onDisappear {
            logStorage.clearLogs()
            LogStorage.shared = nil
        }
    }
}
